...
class AddUserQueueTask extends MessageEncrypte
    with Console, CommonTool, ClientTool {
  // 私有构造函数
  AddUserQueueTask._internal();

  // 静态私有变量，但不立即初始化
  static AddUserQueueTask? _instance;

  // 工厂构造函数
  factory AddUserQueueTask() {
    _instance ??= AddUserQueueTask._internal();
    return _instance!;
  }

  // 执行一次webscoket serverbus消息队列任务调度一次
  Future<void> execOnceWebsocketServerAddUserBusQueueScheduleTask() async {
    // 执行离线消息队列处理消息
    ...
    while (index < clientCount) {
      ...
      if (clientObject.connected && clientObject.status == 1) {
        ...
        if (clientObject.awaitAddFriendsQueue.length == 0) {
          printWarn(
              "由于该clientOdject的addUserFriends队列为空，故忽略该 client: ${clientObject.ip}:${clientObject.port} connect=${clientObject.connected} status=${clientObject.status}");
          // 消息队列为空
          index++; // 增加 index 避免死循环
          continue;
        }
        ...
        Map? msg_map = GlobalManager
            .onlineClientList[index].awaitAddFriendsQueue
            .dequeue();

        // 解密消息: 这里用户自定义核心info
        Map<String, dynamic>? deReMsgInfo =
            decodeMessage(clientObject.secret!, msg_map?["info"])
                .cast<String, dynamic>();

        printSuccess("解密消息: $deReMsgInfo");
        // 2.获取接受者的deviceId
        String receive_deviceId = deReMsgInfo["recipient"]["id"];
        ...
        if (receive_clientObject == null ||
          ...
          if (await OffLine()
              .enOffLineQueue(clientObject.deviceId!, "addUser", msg_map!)) {
          ...
          if (receive_clientObject.status == 1) {
            // 4.加密消息
            msg_map?["info"] =
                encodeMessage(receive_clientObject.secret!, deReMsgInfo);
            // 5.发送
            try {
              ...
              printCatch(
                  "ADDUSER: send to ${receive_clientObject.ip}:${receive_clientObject.port} deviceId=${receive_clientObject.deviceId} is failure!");
